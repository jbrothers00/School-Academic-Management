...
router.post("/retrieve", utils.extractToken, (req, res) => {
  tokenSchema
    ...
    .then((resultList) => {
      ...
      studentSchema.find().exec().then(function (err, student) {
        if (err) {
          console.log(err);
        } else {
          res.json(student);
...
router.post("/add", utils.extractToken, async(req, res) => {
  const matchingStudents= await studentSchema.find(
...
router.delete("/delete/:id", utils.extractToken, (req, res) => {
  tokenSchema
    ...
    .then((resultList) => {
      ...
      studentSchema.findOneAndDelete({ _id: req.params.id }).exec().then(function (err) {
        if (err) res.json(err);
