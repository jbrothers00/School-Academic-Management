...
router.post("/retrieve", utils.extractToken, (req, res) => {
  tokenSchema
    ...
    .then((resultList) => {
      ...
      teacherSchema.find().exec().then((err, teacher) => {
        if (err) {
          console.log(err);
        } else {
          res.json(teacher);
...
router.post("/add", utils.extractToken, async(req, res) => {
  const matchingTeachers =await teacherSchema.find(
...
router.delete("/delete/:id", utils.extractToken, (req, res) => {
  tokenSchema
    ...
    .then((resultList) => {
      ...
      teacherSchema.findOneAndDelete({ _id: req.params.id }).exec().then((err, teacher) => {
        if (err) {
          res.json(err);
        } else {
